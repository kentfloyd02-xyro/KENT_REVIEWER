...
let selectedSubject = localStorage.getItem("selectedSubject") || null;
...
function loadQuestion() {
    ...
    questionEl.textContent =
        `Question ${currentQuestion - startIndex + 1}: ${q.q}`;
    ...
    scoreEl.textContent = `Score: ${score}/50`;

    // Restore saved answer if exists
    if (savedAnswers[currentQuestion] !== undefined) {
        restoreAnswer(savedAnswers[currentQuestion]);
    }
}
...
function selectAnswer(index) {
    ...
    if (savedAnswers[currentQuestion] !== undefined) return;

    buttons.forEach(btn => btn.disabled = true);

    // Save answer
    savedAnswers[currentQuestion] = index;
    ...
    if (index === q.c) {
        ...
        feedbackEl.innerHTML =
            `<p class='wrong'>Wrong! Correct answer:
...
// Restore saved answer after refresh
function restoreAnswer(index) {
    ...
    if (index === q.c) {
        buttons[index].classList.add("correct");
        feedbackEl.innerHTML =
            "<p class='correct'>GALING AH, KUPAL!</p>";
    } else {
        buttons[index].classList.add("wrong");
        buttons[q.c].classList.add("correct");

        feedbackEl.innerHTML =
            `<p class='wrong'>ENGKK. ENGOT! ETO YUNG TAMA BOBO:
             ${String.fromCharCode(65 + q.c)}. ${q.a[q.c]}</p>`;
    }
...
nextBtn.addEventListener("click", () => {
    ...
    // After 50 questions
    if (currentQuestion >= startIndex + 50) {
        quizEl.innerHTML = `
